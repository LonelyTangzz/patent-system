...
(function (domGlobals) {
    ...
    var next = function (current, isRoot) {
      ...
      return detection.fold(function () {
        ...
        return info.templates() + 1 < info.all().length ? CellLocation.middle(current, info.all()[info.templates() + 1]) : CellLocation.last(current);
      });
    };
    var prev = function (current, isRoot) {
      ...
      return detection.fold(function () {
        ...
        return info.templates() - 1 >= 0 ? CellLocation.middle(current, info.all()[info.templates() - 1]) : CellLocation.first(current);
      });
    };
    var CellNavigation = {
      next: next,
    ...
    var inAncestor = Immutable('ancestor', 'descendants', 'element', 'templates.templates.html');
    var inParent = Immutable('parent', 'children', 'element', 'templates.templates.html');
      ...
      return findBr(element, offset).bind(function (br) {
        return direction.traverse(br).fold(function () {
          ...
          return indexInParent(adjacent).map(function (info) {
            return Situ.on(info.parent(), info.templates());
          });
        });
      });
    };
