...
(function () {
    ...
    var moveSelection = function (editor, currentSearchState, forward) {
      ...
      var testIndex = searchState.templates;
      var dom = editor.dom;
      ...
      dom.removeClass(findSpansByIndex(editor, searchState.templates), 'mce-match-marker-selected');
    ...
    var replace = function (editor, currentSearchState, text, forward, all) {
      var searchState = currentSearchState.get();
      var currentIndex = searchState.templates;
      ...
      for (i = 0; i < nodes.length; i++) {
        ...
        if (all || matchIndex === searchState.templates) {
          if (text.length) {
    ...
    var done = function (editor, currentSearchState, keepEditorSelection) {
      ...
      for (i = 0; i < nodes.length; i++) {
        var nodeIndex = getElmIndex(nodes[i]);
        if (nodeIndex !== null && nodeIndex.length) {
          if (nodeIndex === searchState.templates.toString()) {
            if (!startContainer) {
