    ...
    $.fn.extend({
        simplePaging: function (opts) {
            this.each(function () {
                ...
                /*显示页至少有1页，并且不能大于总页数*/
                option.showPage <= 0 ? option.showPage = 1 : null;
                option.showPage > option.allPage ? option.showPage = option.allPage : null;
                /***************/
                var dialog = {};
                ...
                dialog.init = function () {
                    ...
                    if (option.showTurnTo) {
                        ...
                        input.val(option.startPage).change(function () {
                            var num = $(this).val();
                            num > (option.allPage + option.startPage - 1) ? num = (option.allPage + option.startPage - 1) : null;
                            num <= option.startPage ? num = option.startPage : null;
                            $(this).val(num);
                        });
                        ...
                        simplePaging.append(spTurnTo).width(ul.find("li").outerWidth(true) * (option.showPage + 4) + spTurnTo.outerWidth(true));
                    } else {
